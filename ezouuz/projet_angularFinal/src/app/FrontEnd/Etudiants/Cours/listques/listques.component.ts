...
ngOnInit(): void {
  console.log('on init ');
  this.getUserData();
  console.log("userr lkhra",this.currentiduser)
  ...
  if (this.id) {
    ...
    this.getTentativesByUserIdAndQuizId(this.userId, this.id).subscribe((quizPassed: boolean) => {
      if (quizPassed) {
        this.message = 'Vous avez déjà passé ce quiz et il est impossible de le repasser.';
...
getUserData() {
  ...
  terminerQuiz(): void{
    ...
    this.nouvelleTentative.User.id = this.currentiduser ;
...
generatePDF(): void {
  ...
  // Title section
  doc.setFillColor(77, 77, 200); // Blue color for the title background
  doc.roundedRect(10, y, 190, 30, 3, 3, 'FD'); // Title background rectangle
  ...
  y += 50; // Vertical spacing after the title section
