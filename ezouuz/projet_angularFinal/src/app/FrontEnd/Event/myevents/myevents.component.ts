...
export class MyeventsComponent implements OnInit {
  ...
  fetchEvents(id:number) {
    ...
    // Assuming your eventService has a method to fetch events
  ...
  createEvent(Id: number, eventForm: NgForm) {
    if (eventForm.valid) {
      ...
      console.error('Form is invalid. Cannot create event.');
    }
  }
  ...
  onSave() {
    ...
    const updateUrl = `http://localhost:8089/SKyTeck/event/update/${this.editForm.value.numEvent}`;
  ...
  async onFileSelected(event: any) {
    ...
    if (files && files.length > 0) {
      try {
        ...
        // For example, you can set a flag to display an error message in the UI
